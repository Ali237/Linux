...
int main (int argc, char **argv) {
    ...
    char		buf[bufsize];
    int			done[N];
    struct pollfd 	pollfd[N];  // three diff struct each with it's fd events and returned events 

	// first loop for creating the needed pipes
    for (i= 0; i < N; i++)
    	if (pipe(fd[i]) < 0) { perror ("pipe error \n"); exit (1); }   // create the necessary pipes between the clients and server 
	// caling the fork write 3 tiems 
    for (i= 0; i < N; i++)  pid[i]= fork_and_write(i);
	 //   printf ("debug: main: forking writer %d, fd0= %d, fd1= %d\n",
    	 // 		i, fd[i][0], fd[i][1]);

    for (i= 0; i < N; i++) {
    	close (fd[i][1]);			/* parent won't write  only reads*/
    	done[i]= 0;                // intialize the done flag
		// fill in the pollfd strcut 
    	pollfd[i].fd= fd[i][0];    // read in pipe fd
		pollfd[i].events= POLLIN;  // checking the event redy to to read fd is ready to be read 
		pollfd[i].revents= 0;      // always set by the kernal not user 
